...
public class EntityMoveHelper extends MoveControl
{
    private float maxRotate = 90;

    public EntityMoveHelper(Mob entitylivingIn, float maxRotate)
    {
    ...
    public void tick()
    {
        if (this.operation == EntityMoveHelper.Operation.STRAFE)
        {
            ...
            this.operation = EntityMoveHelper.Operation.WAIT;
        }
        else if (this.operation == EntityMoveHelper.Operation.MOVE_TO)
        {
            this.operation = EntityMoveHelper.Operation.WAIT;
            ...
            if (d2 > (double)this.mob.maxUpStep() && d0 * d0 + d1 * d1 < (double)Math.max(1.0F, this.mob.getBbWidth()))
            {
                ...
                this.operation = EntityMoveHelper.Operation.JUMPING;
            }
        }
        else if (this.operation == EntityMoveHelper.Operation.JUMPING)
        {
            this.mob.setSpeed((float)(this.speedModifier * this.mob.getAttribute(Attributes.MOVEMENT_SPEED).getValue()));
