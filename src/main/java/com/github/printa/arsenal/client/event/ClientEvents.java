...
public class ClientEvents {
    ...
    public static MultiBufferSource.BufferSource getDelayedRender() {
        if (DELAYED_RENDER == null) {
            ...
            for (RenderType type : new RenderType[]{
                    CMRenderTypes.VAPOR_TRANSLUCENT,
                    CMRenderTypes.DELAYED_PARTICLE,
                    CMRenderTypes.GLOWING_PARTICLE,
                    CMRenderTypes.GLOWING_BLOCK_PARTICLE,
                    CMRenderTypes.GLOWING,
                    CMRenderTypes.GLOWING_SPRITE}) {
    ...
    public static void onRenderLast() {
            ...
            getDelayedRender().endBatch(CMRenderTypes.DELAYED_PARTICLE);
            RenderSystem.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE);
            getDelayedRender().endBatch(CMRenderTypes.GLOWING_PARTICLE);
            getDelayedRender().endBatch(CMRenderTypes.GLOWING_BLOCK_PARTICLE);
            viewStack.popPose();
            RenderSystem.applyModelViewMatrix();

            getDelayedRender().endBatch(CMRenderTypes.GLOWING_SPRITE);
            getDelayedRender().endBatch(CMRenderTypes.GLOWING);
